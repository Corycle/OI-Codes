...
ll Ask(ll l,ll r,ll c){
	...
	if(x==y)for(ll i=l;i<=r;i++)ans+=(a[i]+dlt[pos[i]]<c);
	else{
		for(ll i=l;i<=ed[x];i++)ans+=(a[i]+dlt[pos[i]]<c);
		for(ll i=st[y];i<=r;i++)ans+=(a[i]+dlt[pos[i]]<c);
		for(ll i=x+1;i<=y-1;i++){
			ans+=max(0ll,lower_bound(t+st[i],t+ed[i]+1,c-dlt[i])-t-st[i]);
		}
	}
	return ans;
}
