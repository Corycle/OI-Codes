...
struct Tree{
	int Min,s1,s2,tag1,tag2;
	void Init(int x){Min=s1=s2=x;}
	void And(int x){Min&=x;s1&=x;s2&=x;tag1&=x;tag2&=x;}
	void Or(int x){Min|=x;s1|=x;s2|=x;tag1|=x;tag2|=x;}
};
struct Segment_Tree{
	Tree t[N*4];
	void Pushup(int p){
