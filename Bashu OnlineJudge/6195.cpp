/*====Corycle====*/
#include<algorithm>
#include<iostream>
#include<iomanip>
#include<cstring>
#include<cstdlib>
#include<complex>
#include<cstdio>
#include<vector>
#include<cmath>
#include<ctime>
#include<stack>
#include<queue>
#include<map>
#define ll long long
using namespace std;
const int inf=0x3f3f3f3f;
const int N=100005;
const int M=55;
int read(){
	int s=0,f=1;char c=getchar();
	while(c<'0'||c>'9'){if(c=='-')f=-1;c=getchar();}
	while(c>='0'&&c<='9'){s=s*10+c-'0';c=getchar();}
	return s*f;
}
int dx[4]={1,0,-1,0};
int dy[4]={0,1,0,-1};
int n,m,K,tot,Ans,cnt=1;
int h[N],vis[N],dis[N],pre[N];
int a[M][M],b[M][M],id[M][M][2];
struct edge{int to,next,flow,cost;}d[N];
void Add(int x,int y,int f,int c){
	d[++cnt]=(edge){y,h[x],f,c};h[x]=cnt;
	d[++cnt]=(edge){x,h[y],0,-c};h[y]=cnt;
}
bool SPFA(int s,int t){
	for(int i=1;i<=tot;i++){dis[i]=inf;vis[i]=pre[i]=0;}
	queue<int>q;q.push(s);dis[s]=0;
	while(q.size()){
		int x=q.front();q.pop();vis[x]=0;
		for(int i=h[x];i;i=d[i].next){
			int y=d[i].to;
			if(d[i].flow&&dis[y]>dis[x]+d[i].cost){
				dis[y]=dis[x]+d[i].cost;pre[y]=i;
				if(!vis[y]){vis[y]=1;q.push(y);}
			}
		}
	}
	return (dis[t]<0);
}
void Adjust(int s,int t){
	int dlt=inf;
	for(int i=t;i!=s;i=d[pre[i]^1].to){
		dlt=min(dlt,d[pre[i]].flow);
	}
	for(int i=t;i!=s;i=d[pre[i]^1].to){
		d[pre[i]].flow-=dlt;
		d[pre[i]^1].flow+=dlt;
	}
	Ans+=dlt*dis[t];
}
void Edmonds_Karp(int s,int t){
	while(SPFA(s,t))Adjust(s,t);
}
int main(){
//	freopen("_.in","r",stdin);
//	freopen("_.out","w",stdout);
	n=read();m=read();K=read();
	int S=++tot,T=++tot,P=++tot;
	for(int i=1;i<=n;i++){
		for(int j=1;j<=n;j++){
			a[i][j]=read();
			id[i][j][0]=++tot;
			if((i+j)&1)id[i][j][1]=++tot;
			Ans+=a[i][j];
		}
	}
	for(int i=1;i<=K;i++){
		int x=read(),y=read();
		b[x][y]=1;
	}
	for(int i=1;i<=n;i++){
		for(int j=1;j<=n;j++){
			if(b[i][j])continue;
			if((i+j)&1){
				Add(id[i][j][0],id[i][j][1],1,-a[i][j]);
				for(int k=0;k<4;k++){
					int x=i+dx[k],y=j+dy[k];
					if(x<1||x>n||y<1||y>n||b[x][y])continue;
					if((x&1)==0)Add(id[x][y][0],id[i][j][0],1,0);
					if((x&1)==1)Add(id[i][j][1],id[x][y][0],1,0);
				}
			}
			else{
				if((i&1)==0)Add(S,id[i][j][0],1,0);
				if((i&1)==1)Add(id[i][j][0],P,1,0);
			}
		}
	}
	Add(P,T,m,0);
	Edmonds_Karp(S,T);
	printf("%d",Ans);
	return 0;
}